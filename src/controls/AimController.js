...
export class AimController {
  constructor(scene, cueBall, camera) {
    ...
    // ✅ لازم نعطي الـ geometry نقطتين افتراضيتين قبل أي استدعاء لـ computeLineDistances
    ...
    this.line.computeLineDistances(); // هلق آمن لأنه في نقاط فعلية
  ...
  update(mouseNDC, raycaster, tableBounds, allBalls) {
    ...
    if (!hit) return; // الماوس خارج مستوى الطاولة — تجاهل هالإطار

    this.aimDirection.subVectors(target, this.cueBall.mesh.position).setY(0);
    if (this.aimDirection.lengthSq() < 1e-8) return; // تجنّب تطبيع متجه صفري
    this.aimDirection.normalize();

    const points = this.traceTrajectory(
      this.cueBall.mesh.position.clone(), this.aimDirection, tableBounds, allBalls
    );

    // ✅ لازم نتأكد إنه عندنا نقطتين ع الأقل قبل ما نبني geometry جديدة
    ...
    if (points.length > 1) {
      this.ghostBallMesh.position.copy(points[1]).addScaledVector(this.aimDirection, -0.0571);
  ...
  traceTrajectory(origin, direction, bounds, allBalls) {
    ...
    for (let bounce = 0; bounce <= this.maxBounces; bounce++) {
      ...
      for (const ball of allBalls) {
        ...
        if (proj < 0) continue;
        const closestPoint = currentPos.clone().addScaledVector(currentDir, proj);
        ...
        if (distSq < combinedRadiusSq) {
          ...
          if (t < nearestT && t > 0) { nearestT = t; hitBall = ball; }
        }
      }
      ...
      if (wallT < nearestT && wallT !== Infinity) {
        ...
        currentDir = this.reflect(currentDir, this.getWallNormal(wallPoint, bounds));
  ...
  getWallNormal(point, bounds) {
    const eps = 0.005;
    if (Math.abs(point.x - bounds.minX) < eps) return new THREE.Vector3(1, 0, 0);
    if (Math.abs(point.x - bounds.maxX) < eps) return new THREE.Vector3(-1, 0, 0);
    if (Math.abs(point.z - bounds.minZ) < eps) return new THREE.Vector3(0, 0, 1);
