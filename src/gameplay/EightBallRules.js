export class EightBallRules {
  ...
  onContact({ idA, idB, type }) {
    if (type === "BALL_BALL" && this.shotLog.firstContact === null) {
      const objectBallId = idA === 0 ? idB : idA;
  ...
  onPocketed({ ballId }) {
    if (ballId === 0) {
      this.shotLog.cueScratched = true;
      return;
    }
  ...
  evaluateShot() {
    ...
    if (eightPocketed) {
      ...
      if (!legallyOnEight || this.shotLog.cueScratched) {
        gameOverInfo = {
          winner: this.turnManager.otherPlayer(currentPlayer),
          reason: "8-ball foul",
        };
      } else if (legallyOnEight && !foul) {
        gameOverInfo = {
          winner: currentPlayer,
          reason: "8-ball legally pocketed",
        };
      }
    }
    ...
    if (foul) {
      this.eventBus.emit("foul:committed", {
      ...
      const pocketedOwnGroup = this.shotLog.pocketed.some((id) =>
        this.ballBelongsToGroup(id, legalGroup),
      );
    ...
    if (ballInHandRequested) {
      this.eventBus.emit("request:ball-in-hand", {
        player: this.turnManager.currentPlayer,
      });
    }
  ...
  detectFoul(player) {
    ...
    if (this.groupsAssigned && group) {
      const isOwnBall = this.ballBelongsToGroup(
        this.shotLog.firstContact,
        group,
      );
      const onEightBall = this.isPlayerOnEightBall(player);
      if (onEightBall && this.shotLog.firstContact !== 8)
        return "WRONG_BALL_FIRST";
      if (!onEightBall && !isOwnBall && this.shotLog.firstContact !== 8)
        return "WRONG_BALL_FIRST";
    }

    if (
      this.shotLog.pocketed.length === 0 &&
      this.shotLog.railsHitAfterContact === 0
    ) {
  ...
  assignGroups(player, pocketedIds) {
    ...
    this.playerGroups[this.turnManager.otherPlayer(player)] =
