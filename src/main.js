...
async function boot() {
  ...
  const assetLoader = new AssetLoader((p) => {
    document.getElementById("loading-bar-fill").style.width = `${p * 100}%`;
  });
  ...
  const cueBall = new Ball(
    ...
    physicsWorld,
    0,
    new THREE.Vector3(0, 0.028575, 0.55),
  );
  balls.push(cueBall);
  ...
  const spinSelector = new SpinSelector(
    document.getElementById("spin-selector-canvas"),
  );
  const ballInHandController = new BallInHandController(
    cueBall,
    table,
    raycaster,
  );
  ...
  function fireCueShot(impulseMagnitude) {
    ...
    eventBus.emit("physics:contact", {
      idA: -2,
      idB: 0,
      type: "BALL_CUE",
      impactVelocity: impulseMagnitude,
    });
    cue.hide();
    spinSelector.reset();
  ...
  canvas.addEventListener("mousemove", () => {
    ...
    ballInHandController.onPointerMove(
      inputManager.mouseNDC,
      sceneManager.camera,
      balls.filter((b) => b !== cueBall && !b.isPocketed),
    );
  });
  ...
  eventBus.on("shot:evaluated", (result) => {
    result.pocketed.forEach((id) =>
      balls.find((b) => b.id === id)?.setPocketed(),
    );
    if (result.cueScratched)
      cueBall.respawnAt(new THREE.Vector3(0, 0.028575, 0.55));
    if (result.gameOver) {
      fsm.transition(States.GAME_OVER);
      return;
    }
    fsm.transition(
      result.requestBallInHand ? States.BALL_IN_HAND : States.AIMING,
    );
  });
  ...
  engine.onUpdate((deltaTime) => {
    ...
    physicsWorld.contactEvents.forEach((evt) =>
      eventBus.emit("physics:contact", evt),
    );
...
boot().catch((err) => {
  ...
  document.getElementById("loading-text").textContent =
    "حدث خطأ أثناء التحميل. راجع الكونسول.";
});
