...
export class PhysicsWorld {
  ...
  addCushionSegment(a, b) {
    this.cushionSegments.push({ a: a.clone(), b: b.clone() });
  }
  setSpinSolver(spinSolver) {
    this.spinSolver = spinSolver;
  }
  ...
  _substep(dt) {
    const activeBalls = this.bodies.filter(
      (b) => !b.isPocketed && !b.rigidBody.isFalling,
    );
    ...
    while (remaining > 1e-9 && passes < this.maxCollisionPassesPerSubstep) {
      ...
      if (!earliest) {
        activeBalls.forEach((b) =>
          b.rigidBody.position.addScaledVector(b.rigidBody.velocity, remaining),
        );
        remaining = 0;
        break;
      }
      ...
      activeBalls.forEach((b) =>
        b.rigidBody.position.addScaledVector(b.rigidBody.velocity, earliest.t),
      );
        ...
        if (dist < A.radius + B.radius) {
          ...
          if (result) {
            this.contactEvents.push({
              ...
              idB: balls[j].id,
              type: "BALL_BALL",
              impactVelocity: result.impactSpeed,
            });
          }
        }
      }
    ...
    for (const ball of balls) {
      ...
      for (const seg of this.cushionSegments) {
        ...
        const delta = new THREE.Vector3(
          rb.position.x - closest.x,
          0,
          rb.position.z - closest.z,
        );
    ...
    for (let i = 0; i < balls.length; i++) {
      for (let j = i + 1; j < balls.length; j++) {
        ...
        if (t !== null && (!best || t < best.t))
          best = { type: "BALL_BALL", t, a: A, b: B };
      }
    }

    for (const ball of balls) {
      for (const seg of this.cushionSegments) {
        const t = sweptSphereSegmentTime(
          ball.position,
          ball.velocity,
          ball.radius,
          seg.a,
  ...
  _resolveEvent(event) {
    if (event.type === "BALL_BALL") {
      const result = resolveBallBallCollision(
        event.a.rigidBody,
        event.b.rigidBody,
        this.spinSolver,
      );
      ...
      const normal = new THREE.Vector3(
        event.ball.position.x - closest.x,
        0,
        event.ball.position.z - closest.z,
      ).normalize();
      const result = resolveBallCushionCollision(
        event.ball.rigidBody,
        normal,
        0,
      );
      if (result.impactSpeed > 0) {
